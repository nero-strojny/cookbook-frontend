...
import {Button, Card, Grid, Loader, Pagination, SemanticWIDTHSNUMBER, Transition} from "semantic-ui-react";
import RecipeCard from "./RecipeCard";
import {defaultPaginatedRequest, getRandomRecipes, getRecipes} from "../serviceCalls";
      ...
      if (isCurrent) {
        if (serverState.shouldRefresh) {
          ...
          if (response.status === 200) {
            setErrorState("");
            serverDispatch({
              type: 'QUERY_RECIPES_SUCCESS',
              payload: {recipes: response.data.recipes, numberOfRecipes: response.data.numberOfRecipes}
            });
